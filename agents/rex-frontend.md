...
Frontend ADR considerations: state ownership (TanStack Query vs Zustand vs URL), auth-token storage, response-shape contracts. If the ADR's frontend rules conflict with shadcn/ui patterns, follow the ADR — flag the friction back to Rex.
